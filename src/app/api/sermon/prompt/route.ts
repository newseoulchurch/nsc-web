...
export async function GET(req: Request) {
  ...
  const completionRes = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: 'You are a helpful assistant. Given a user query, rewrite it as a search query to find sermons that could assist or encourage the user based on their emotional or spiritual need.' },
        { role: 'user', content: q }
      ],
      max_tokens: 50,
    }),
